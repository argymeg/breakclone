...
getScore <- function(pair, segmentTable, abg = NULL){
  ...
  sample1_lists <- c(list(), list(sample1["loss", on = "status"]), list(sample1["cnloh", on = "status"]), list(sample1["gain", on = "status"]), list(sample1["amp", on = "status"]))
  sample2_lists <- c(list(), list(sample2["loss", on = "status"]), list(sample2["cnloh", on = "status"]), list(sample2["gain", on = "status"]), list(sample2["amp", on = "status"]))
  ...
  ndiscordant <- 2 * sum(unlist(lapply(c(sample1_granges, sample2_granges), length))) - sum(unlist(lapply(c(hits_start, hits_end), length)))
